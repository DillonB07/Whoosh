...
/**
 * `Surface` is a container component, similar to `View`. However, `Surface` should be used for setting the `elevation` for a contained section of UI. For normal containers that do not need to set or change elevation, use `View`.
 *
 * You can set elevation explicitly with the `background` prop. The value can be `root`, `default`, `higher`, or `highest`. `Elevation` affects the background color of the `Surface`, but will also influence the color of children that take advantage of the `Interactive` utilities, like `Button` and `Pill`, which will render contextually based on the `elevation`.
 *
 * You can also set the boolean prop `elevated`, which will automatically set the `Surface`’s `elevation` to be one level above any parent `Surfaces` it is in.
 */
